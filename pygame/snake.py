...
def get_current_direction():
    ...
    elif head_y - 1 == first_segment_x: return "down"
    elif head_y + 1 == first_segment_x: return "up"
...
def check_turn_around(direction, food_dir):
    org_direction = direction
    for n in range(1, len(snake.segments)):
        if direction == "left":
            print("left",snake.segments[0].Y, snake.segments[n].Y)
            if snake.segments[0].Y < snake.segments[n].Y:
                direction = "up"
            else:
                direction = "down"
        elif direction == "right":
            print("right", snake.segments[0].Y, snake.segments[n].Y)
            if snake.segments[0].Y < snake.segments[n].Y:
                direction = "up"
            else:
                direction = "down"
        elif direction == "up":
            print("up", snake.segments[0].X, snake.segments[n].X)
            if snake.segments[0].X < snake.segments[n].X:
                direction = "left"
            else:
                direction = "right"
        elif direction == "down":
            print("down", snake.segments[0].X, snake.segments[n].X)
            if snake.segments[0].X < snake.segments[n].X:
                direction = "left"
            else:
                direction = "right"
    ...
    return direction                                
def auto_move():
    old_direciton = direction = get_current_direction()
    food_dir = get_food_direction()
    print("original: ",direction, food_dir)
    if food_dir == "left":
        if direction != "right":
            direction = "left"
        else: 
            direction = check_turn_around(direction, food_dir)
    if food_dir == "right":
        if direction != "left":
            direction = "right"
        else:
    ...
    #for n in range(1, len(snake.segments)):
    #    if snake.segments[0].X + snake.velocity.x * 32 == snake.segments[n].X and snake.segments[0].Y + snake.velocity.y * 32 == snake.segments[n].Y:
    #        direction = old_direciton
    #        print("keep direciton")
